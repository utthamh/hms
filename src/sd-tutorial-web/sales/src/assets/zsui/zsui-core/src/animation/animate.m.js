...
var animation = {
	 ...
	 * @see zs.transition
	 */
	registerTransition: function (name, params) {
	...
	transition: function (params) {
		...
		var reset = effect.reset;

		// Set initial state with custom options
		Object.assign(this.el.style, before, options.before);

		// Set duration
		this.el.style.transitionDuration = parseFloat(this.duration / 1000).toFixed(2) + 's';

		// Go to the final state and apply custom options
	 ...
	 */
	start: function (params, duration) {
		var animation = this;
		...
		var prom = new Promise(function (resolve, reject) {
			setTimeout(function () {
				if (animation.el) {
					Object.assign(animation.el.style, { transition: 'none' }, animation.currentEffect.reset);
				}
				resolve(animation);
...
var animate = {
	 ...
	 * @see zs.animation
	 */
	registerAnimation: function (effectName, duration, options) {
		if (!this.animations) { this.animations = {}; }
		var animation = Object.assign(Object.create(zs.animation), { effect: effectName, duration: duration, options: options, el: this });
	 ...
	 */
	startAnimation: function (effectName, duration, options) {
		if (!this.animations) { this.animations = {}; }
...
export default { transition, animation, animate };