...
var tooltip = {
	...
	events: {
		create: function () {
			...
			// store them to be able to remove listeners in future
	 ...
	 * @private
	 */
	_defineAnchor: function () {
		if (this.anchor) { // if anchor was already defined skip it
			return this;
		}

		if (!this.for) { // if user does not specified 'for' selector use paren node for anchor
	 ...
	 * @private
	 */
	_whenAnimationFinished: function (e) {
	 ...
	 * @private
	 */
	_finalizeAfterShowHide: function () {
...
var tooltipElement = zs.customElement(HTMLElement, "zs-tooltip", null, [tooltip, zs.animate]);

// Add tooltip behaviors to zs namespace for backward compatibility.
window.zs = window.zs || {};
window.zs.tooltip = tooltip;
window.zs.tooltipElement = tooltipElement;

export default { tooltip, tooltipElement };
