describe("FetchService", function () {
	var url = 'https://jsonplaceholder.typicode.com/users',myAPI,
		FetchService = window.fetchServiceM.default;


	var isIE11 = !(window.ActiveXObject) && "ActiveXObject" in window;

	it('creates an instance of a service that wraps Fetch API', function(done) {
		expect(typeof FetchService).toBe('function');
		myAPI = new FetchService({
			url: url
		});
	...
	it('can build URL from parameters', function() {
		...
		expect(myAPI.url).toBe('https://jsonplaceholder.typicode.com/users/1?userId=1');
	});
	...
	it('supports POST', function(done) {
		...
		myAPI.post().then(myAPI.response).then(function(data) {
	...
	it('supports PUT', function(done) {
		...
		myAPI.put().then(myAPI.response).then(function(data) {
	...
	it('supports PATCH', function(done) {
		...
		myAPI.patch().then(myAPI.response).then(function(data) {
	...
	it('supports DELETE', function(done) {
		...
		myAPI.delete().then(myAPI.response).then(function(data) {
			expect(data).toBeTruthy();				
  		  	done();
		}).catch(function(err) {
			if (isIE11) {
	...
	it('handles errors', function(done) {
		...
		myAPI.request().then(myAPI.response).then(function(data) {
