(function (global, factory) {
  ...
  var l10n = {
     ...
     */
    localize: function localize(value, options) {
      if (value == null) {
        return '';
      } // null and undefined	


      var typeStr = _typeof(value);

      if (typeStr == 'number' && isNaN(value)) {
        return '';
      } // NaN
      ...
      var methodName = "".concat(typeStr, "ToLocaleString");
     ...
     */
    stringToLocaleString: function stringToLocaleString(str, locale, options) {
     ...
     * @returns {string} Translated string
     */
    booleanToLocaleString: function booleanToLocaleString(bool, locale, options) {
    ...
    format: function format(value, options) {
      ...
      var methodName = "format".concat(typeStr);
    ...
    */
    formatString: function formatString(value, locale, options) {
     ...
     */
    formatNumber: function formatNumber(value, locale, options) {
      locale = locale || this.locale;
     ...
     */
    formatDate: function formatDate(value, locale, options) {
     ...
     */
    parse: function parse(stringValue, valueType, options) {
      if (stringValue === null) {
        return null;
      } // null 
      ...
      var methodName = "parse".concat(valueType);
     ...
     */
    parseNumber: function parseNumber(stringValue, locale, options) {
      locale = locale || this.locale; // Detect separator

      var str = this.localize(10.2); // 10,2 for de-DE

      var arr = str.split(/[0-9]+/ig); // "10,2" => ["", ",", ""]  ",2" => [",", ""] 
     ...
     */
    parseBoolean: function parseBoolean(stringValue, locale, options) {
      locale = locale || this.locale;

      if (stringValue == this.localize('true')) {
        return true;
      }

      if (stringValue == this.localize('false')) {
        return false;
      }
     ...
     */
    parseDate: function parseDate(stringValue, locale, options) {
      locale = locale || this.locale;
